...
#  added 
from ml.rl.models.dqn import CNNDQN
...
def create_dqn_trainer_from_params(
    ...
    ###################################
    # check clause added 102719 for BCQ 
    ###################################
    elif model.rainbow.bcq:
        print("Under BCQ...")
        # Pre-train a GBDT imitator if doing batch constrained q-learning in Gym
        assert model.rainbow.gbdt_trees is not None
        gbdt = GradientBoostingClassifier(
            n_estimators=model.rainbow.gbdt_trees,
            max_depth=model.rainbow.max_depth,
            verbose=1,
            n_iter_no_change=10
        )
        file = model.rainbow.training_data_path
        X = []
        y = []
        with open(file, "r") as f:
            for step, l in enumerate(f):
                l = json.loads(l.replace("\n",""))
                if len(list(pd.DataFrame.from_dict(l["state_features"], orient="index").values.reshape(-1))) == 375: 
                    X.append(list(pd.DataFrame.from_dict(l["state_features"], orient="index").values.reshape(-1)))
                    y.append(l["action"])
                if step % 5000 == 0:
                    print(step)
                if step > 50000:
                    break 
        print("Fitting GBDT for Imitator with sample of ", len(X))      
        ...
        gbdt.fit(list(X_train), y_train)
        # gbdt_predict = np.array(gbdt.predict_proba(X_test)).reshape(-1)
        # print('predict info:', gbdt_predict.mean(), gbdt_predict.min(), np.quantile(gbdt_predict, 0.1), np.quantile(gbdt_predict, 0.2), np.quantile(gbdt_predict, 0.5))
