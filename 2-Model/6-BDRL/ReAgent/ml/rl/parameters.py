...
class RainbowDQNParameters(BaseDataClass):
    ...
    bcq_drop_threshold: float = 0.05
        
    # added the following lines to pass parameters to train imitator
    gbdt_trees: int = 10
    max_depth: int = 2
...
@dataclass
class LiveRunDetails(BaseDataClass):
    solved_reward_threshold: Optional[int] = None
    max_episodes_to_run_after_solved: Optional[int] = None
    stop_training_after_solved: bool = False
    num_episodes: int = 301
...
@dataclass
class LiveParameters(BaseDataClass):
    env: str
