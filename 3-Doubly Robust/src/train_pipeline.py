...
def mrdr_train(samples, qnet, optimizer, loss_mode, config, permutation, i_batch, wis_reward=False):
    ...
    if loss_mode == 's': # stochastic: add some noise into pie to create a biased but lower variance target
        ...
        qa_values = q_values.gather(1, actions).squeeze()
        loss = torch.nn.MSELoss()(qa_values, acc_rewards.cpu())
...
def mdpmodel_train(memory, mdpnet, optimizer, loss_mode, config):
    ...
    expanded_actions = expanded_actions.expand(-1,-1,config.state_dim) # batch_size x 1 x state_di
    predict_state_diff, predict_reward_value, rep = mdpnet(states.type(Tensor).cpu())
    # predict_state_diff = batch_size x 2 x state_dim
    predict_state_diff = predict_state_diff.gather(1, expanded_actions.cpu()).squeeze() # batch_size x state_dim
    predict_reward_value = predict_reward_value.gather(1, actions.cpu()).squeeze()
    ...
    if sum_factual > 0 and sum_control > 0:
        # factual_index = LongTensor([i for i, t in enumerate(transitions) if t.factual[0] == 1])  #changed on Oct 2, 2019
        # last_factual_index = LongTensor([i for i, t in enumerate(transitions) if t.last_factual[0] == 1])#changed on Oct 2, 2019
        factual_index = LongTensor([i for i, t in enumerate(transitions) if t.factual[0] <100])#changed on Oct 2, 2019, 100 is a arbitrary number
        last_factual_index = LongTensor([i for i, t in enumerate(transitions) if t.last_factual[0] <100])#changed on Oct 2, 2019, 100 is a arbitrary number
        factual_rep = rep.index_select(0,factual_index.cpu())
        control_rep = rep.index_select(0,last_factual_index.cpu())
    ...
    if loss_mode == 0: # MSE_mu
        loss = torch.nn.MSELoss()(predict_state_diff, states_diff.cpu()) \
               + torch.nn.MSELoss()(predict_reward_value, reward.cpu())
    elif loss_mode == 1: # MSE_pi + MSE_mu (objective in RepBM paper) MDPnet
        loss = weighted_mse_loss(predict_state_diff, states_diff.cpu(), weights.cpu()) \
               + weighted_mse_loss(predict_reward_value, reward.cpu(), weights.cpu()) \
...
def mdpmodel_test(memory, mdpnet, loss_mode, config):
    ...
    predict_state_diff, predict_reward_value, rep = mdpnet(states.type(Tensor).cpu())
    predict_state_diff = predict_state_diff.gather(1, expanded_actions.cpu()).squeeze()
    predict_reward_value = predict_reward_value.gather(1, actions.cpu()).squeeze()
    ...
    if loss_mode == 0:
        ...
        loss = weighted_mse_loss(predict_state_diff, states_diff.cpu(), weights_2.cpu()) \
               + weighted_mse_loss(predict_reward_value, reward.cpu(), weights_2.cpu())
    return loss.item()
